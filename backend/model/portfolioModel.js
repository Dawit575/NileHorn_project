...
export const createPortfolio = (data, callback) => {
  ...
  const sql = `
    INSERT INTO portfolio (title, description, image, github_link, live_link)
    VALUES (?, ?, ?, ?, ?)
  `;

  db.query(
    sql,
    [data.title, data.description, data.image, data.github_link, data.live_link],
    callback
  );
};
...
export const updatePortfolio = (id, data, callback) => {
  ...
  const sql = `
    UPDATE portfolio
    SET title = ?, description = ?, image = ?, github_link = ?, live_link = ?
    WHERE id = ?
  `;

  db.query(
    sql,
    [data.title, data.description, data.image, data.github_link, data.live_link, id],
    callback
  );
};
